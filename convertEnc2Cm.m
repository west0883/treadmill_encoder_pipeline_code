function [converted] = convertEnc2Cm(logFile,velocityFlag, radius)
...
for i = 1:n
    ...
    if isempty(x) == 0
        ...
        converted(i).trialNum = i;
        converted(i).positions = positions;
        converted(i).trialTime = data(x,2);
        
        if strcmp(velFlag,'True')
            
            dt = diff(converted(i).trialTime);
            dx = diff(converted(i).positions);          
            converted(i).velocities = dx./dt;
            converted(i).totalDist = sum(dx);
